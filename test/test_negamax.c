 ...
 * the heuristics do not break correctness but cannot detect a silently-neutered
 * implementation. To close this gap: expose a negamax_get_node_count() API and
 * assert fewer nodes are searched with heuristics enabled, or add a
 * -DDISABLE_HEURISTICS compile flag and compare node counts between builds. */
